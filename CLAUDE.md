...
- `Core/Ingest/TranscriptReader.cs` — 读会话 transcript(JSONL)**末尾**最近一条 assistant 记录,取 `message.model` 与 `message.usage`。`FileShare.ReadWrite` 共享读(claude 进程在写),尾部回读 256KB;**找不到 usage 就 4× 扩窗回退**(`ReadWindow`)直至命中或读完整个文件。另从 transcript 重建 `TaskCreate` / `TaskUpdate` 任务清单,因为真实 Claude Code 会话当前不用 `TodoWrite`;hover 热路径用 `TaskSnapshotCursor` 增量读追加部分。
...
- `Core/State/AgentSessionStateMachine.cs` — 纯逻辑状态机:事件转状态、优先级权重、tool-hold 回落、Done/Idle/WaitingApproval/长静默 Thinking/RunningTool prune 规则。不要把这些规则散回 WPF。
- `IslandWindow.xaml(.cs)` — 灵动岛 UI:横条态状态机(工具态 `PostToolUse` 后延迟 ~1.2s 回落,毫秒级工具不被秒杀) + 形变动画 + hover 展开详情卡(顶部状态圆点:有文字一组居中、idle 无文字单独居中)。详情卡内容**全部居中、分两个浅色圆角背景块**(`TaskCard` 任务清单组:进度行 + `RebuildTodoList` 动态生成的居中 todo 行,完成项折叠成计数,无 todo 时整块隐藏;`MetaCard` 元信息组:目录 + 模型/强度/上下文),高度按未完成行数自适应。**多会话「主药丸 + 副会话点」**(M3,2026-06-19):按 `session_id` 分桶(`Dictionary<string, Session>`,Session 加 `Key`/`LastActivity`),`PickActive()` 按**优先级**(`Weight`:WaitingApproval4 > RunningTool3 > Thinking2 > Done1 > Idle0,同权重取最近活动)选主会话驱动主药丸,**不再是「最后一个事件」**;其余非 Idle 会话住进独立**分裂副岛** `SatIsland`(第三版 2026-07-05,PR #2,iOS 双 Live Activity 正宗分裂形态):`IslandRow`(StackPanel)包主岛 + 副岛整组水平居中,两岛间 `SatIslandGap=8` 桌面缝隙;副岛独立 squircle + 阴影,宽度自适应点数(单点 28×28 正圆),内铺 9×9 状态色圆点,超 `MaxSatellites=3` 折叠「+N」(`MakeOverflowDot`),等待批准副点呼吸脉冲吸引注意。出/退场动画:出场布局宽度 0→自然宽 + 从主岛底下弹簧滑出(「分裂」),退场宽度收 0 + 滑入主岛底下(`Pill` ZIndex 在上)+ 缩小淡出(「吞并/合并」);**宽度/边距参与动画 → StackPanel 全程连续重居中,消掉 Collapsed 瞬间主岛跳位的生硬感**(这是前两版被斃的根因之一)。hover 展开时副岛被长大的详情卡吞掉,由详情卡 `SessionTabs` 接管;药丸宽度与副点彻底解耦(删 `SatelliteStripWidth`/`_renderedSatWidth` 记账)。详情卡顶部 `SessionTabs` 列出会话标签(状态点 + 短目录,统一圆角/间距/`MaxWidth=56`,超 `MaxSessionTabs=3` 折叠「+N」且当前 pin 的会话务必在列,点击 `PinSession` 锁定查看),区分**「主药丸=自动优先级」vs「详情卡=用户 pin 想看」**(`_activeRendered`/`_displayed`/`_pinnedKey`);hover 离开清 pin 回自动,pin 的会话被 prune 自动回落。**渲染解耦三层**:`ApplyStatus(Session,...)` 只改数据、`RenderPill(Session?)` 全量重画主药丸(含 null→Idle 外观,宽度不含副点)、`RenderActive()` 调度(PickActive + 脏检查防抖(会话/状态/文字/ambient),之后 `RenderSatellites` 副岛每次全量重建(几颗点,廉价)+ 刷详情卡)。`_pruneTimer`(2s)做生命周期:Done 超 `DoneLingerSeconds=6` 回落 Idle、Idle 超 `IdleEvictSeconds=30` 移除(替代旧单例 `_doneTimer`,天然支持多会话);`_toolHoldTimer` 绑 `_toolHoldTarget` 只对当前主会话回落。**内存守护(关键)**:transcript 只给 `_displayed` 一个会话读(仅 hover 时),**绝不遍历批量拉**。**视觉走 SwiftUI 三件套**(2026-06-19):图标全用单色线性字体(Segoe Fluent Icons + Lucide 补 cpu/bar-chart),经 `AddIconRun` 以 `Inline.Run` 混排(图标字体 + 雅黑文字,颜色随 `Foreground` 得灰阶层级);展开/收起用 `SpringEase` 阻尼弹簧;背景与裁剪走 squircle 连续曲率几何,展开态圆角 `PillRadius` 14→24 动画。
...
- `tests/DynamicIsland.Core.Tests/` — xUnit 回归测试,覆盖状态机陷阱与 transcript 尾窗/任务重建/增量 cursor。
...
- **状态机硬化(2026-07-05 修)**:`Thinking` / `RunningTool` 若终端被杀、崩溃或 hook 后续丢失,同样可能永久占主药丸;现在 `AgentSessionStateMachine.Prune` 对任意活跃静默态加 10 分钟兜底回 Idle。`_toolHoldTimer.Stop()` 只能由同一 `_toolHoldTarget` 会话的新事件触发,避免 B 会话事件误取消 A 会话的工具名回落。状态机规则已搬到 Core 并有 xUnit 覆盖,不要再把分支散写回 `IslandWindow`。
...
- **transcript 缓存别把失败当成功(2026-07-05 修)**:`Session.LastTranscriptLen` 只表示 usage 成功读到时的长度,不能在 `ReadLatest` 前提前写;否则尾部超大/半截行导致一次无 usage 后,长度不变的最终会话会永久跳过重读。任务清单走 `TaskSnapshotCursor` 增量解析追加内容,半截 JSONL 尾行不推进 offset,文件截断时 reset。
...
- **M5 部分 ✅** 托盘 + 退出（2026-07-05）：`NotifyIcon` 右键菜单支持显示/隐藏灵动岛与退出,双击托盘切换显示;退出时 dispose 托盘、停止信息源、释放 `IngestServer`。`UseWindowsForms=true` 会引入 WinForms/System.Drawing 隐式命名空间,新增 WPF 类型别名避免 `Brush/Color/Point/Cursors` 歧义。
- **M5 ⬜** 工程化剩余:点击穿透、多屏 + DPI、全屏自动隐藏、开机自启
